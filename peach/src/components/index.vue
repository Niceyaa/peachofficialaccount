<template>
  ...
  <div class="news" v-wechat-title="$route.meta.title">
    123132
    <div class="news-wrap">
      <ul id="new-ul" v-for="item in list">
        <router-link :to="{name:'newsContent',params:{count: item.cnId}}">
          <li>
            <img :src="http+item.csPictureUrl" alt="title-picture">
            <p>{{item.csContentName}}</p>
          </li>
        </router-link>
      </ul>
    </div>
...
<script>
  ...
  export default{
    ...
    methods:{
      init:function(){
        console.log("ssssss");
        axios.post('http://47.108.56.198:9898/scenery/news/findAll', {
          "cnSceneryId":8,
          "pageNum":1,
          "pageSize":6
        })
